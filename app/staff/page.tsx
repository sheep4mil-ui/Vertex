            ...
            <div className="hiring-admin">
              ...
              {selectedApplication && <article className="panel application-review">
                ...
                <div className="grid2 hiring-controls">
                  <div className="field"><label>Hiring stage</label><div className="hiring-stage-buttons">{["new", "reviewing", "interview"].map((stage) => <button key={stage} type="button" className={`btn ${selectedApplication.status === stage ? "btn-dark" : "btn-light"}`} onClick={() => setSelectedApplication({ ...selectedApplication, status: stage })}>{stage === "new" ? "New" : stage === "reviewing" ? "Reviewing" : "Interview"}</button>)}</div><small>Choose a stage, then save the review progress.</small></div>
