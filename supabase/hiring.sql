...
create or replace function public.submit_vertex_application(
  ...
  p_portfolio_url text, p_reference_info text, p_guardian_permission boolean
) returns text
...
begin
  ...
  insert into public.job_applications(full_name,email,phone,age_range,school_or_program,roles,availability,experience,skills,why_vertex,portfolio_url,reference_info,guardian_permission)
  values(trim(p_full_name),lower(trim(p_email)),nullif(trim(p_phone),''),p_age_range,nullif(trim(p_school_or_program),''),coalesce(p_roles,'{}'),trim(p_availability),trim(p_experience),trim(p_skills),trim(p_why_vertex),nullif(trim(p_portfolio_url),''),nullif(trim(p_reference_info),''),coalesce(p_guardian_permission,false))
...
revoke all on function public.submit_vertex_application(text,text,text,text,text,text[],text,text,text,text,text,text,boolean) from public;
grant execute on function public.submit_vertex_application(text,text,text,text,text,text[],text,text,text,text,text,text,boolean) to anon, authenticated;
