...
create or replace function public.submit_order(p_customer_name text,p_customer_email text,p_customer_phone text,p_update_preference text,p_material text,p_quantity integer,p_details text,p_model_url text)
...
begin
  ...
  insert into public.orders(customer_name,customer_email,customer_phone,update_preference,material,quantity,details,model_url)
  values(trim(p_customer_name),lower(trim(p_customer_email)),nullif(trim(p_customer_phone),''),p_update_preference,p_material,p_quantity,trim(p_details),nullif(trim(p_model_url),''))
...
$$;
revoke all on function public.submit_order(text,text,text,text,text,integer,text,text) from public;
revoke all on function public.track_order(text,text) from public;
grant execute on function public.submit_order(text,text,text,text,text,integer,text,text) to anon,authenticated;
